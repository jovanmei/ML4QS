...
DATA_PATH = Path('../')    
DATASET_FNAME = 'ch3_res_outliers.csv'
RESULT_FNAME = 'ch3_res_final.csv'
ORIG_DATASET_FNAME = 'ch2_res.csv'
...
def main():
    ...
    if FLAGS.mode == 'imputation':
        ...
        selected_predictor_cols = [c for c in dataset.columns if (not ('loc' in c)) and (not ('label' in c)) and 
                                    (not (c == 'simple_dist_outlier'))]
